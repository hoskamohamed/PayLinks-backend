...
class PaymentGateway {
    ...
    public function gateway() {
        ...
        if($this->output['type'] == PaymentGatewayConst::TYPEPAYLINK){
            $data = PaymentLink::with('user')->find($validated['target']);
            $this->output['payment_link']       = $data;
        }else{
            $data = Invoice::with('user','invoiceItems')->find($validated['target']);
            $this->output['invoice']       = $data;
        }
        $user_wallet = UserWallet::with('user','currency')->where('user_id', $data->user_id)->first();
        if(!$user_wallet) {
            throw ValidationException::withMessages([
                $this->currency_input_name = "User wallet not found!",
            ]);
        }
        $sender_currency = ExchangeRate::where('currency_code', $data->currency)->where('currency_name', $data->currency_name)->first();
    ...
    public function chargeCalculate($currency) {
        ...
        if($this->output['type'] == PaymentGatewayConst::TYPEPAYLINK){
            ...
            $data = Invoice::find($this->request_data[$this->target]);
            $amount = $data->amount;
        }

        $receiver_currency = ExchangeRate::where('name', $data->user->address->country)->first();
    ...
    public function responseReceive($type = null) {
        ...
        if($type == 'stripe'){
            ...
            if(method_exists(Flutterwave::class,$method_name)) {
                return $this->$method_name($this->output);
            }
