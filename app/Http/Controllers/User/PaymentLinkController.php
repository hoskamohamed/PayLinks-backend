...
class PaymentLinkController extends Controller
{
    ...
    public function create(){
        ...
        return view('user.sections.payment-link.create', compact('page_title','currency_data', 'payment_gateways'));
    }
    ...
    public function store(Request $request){
        ...
        return redirect()->route('user.payment-link.share', $payment_link->id)->with(['success' => ['Payment Link Created Successful']]);
    }
    ...
    public function update(Request $request){
        ...
        return redirect()->route('user.payment-link.share', $paymentLink->id)->with(['success' => ['Payment Link Updated Successful']]);
    }
    ...
    public function status(Request $request){
        ...
        return redirect()->route('user.payment-link.index')->with(['success' => ['Payment Link Status Updated Successful']]);
    }
    ...
    public function paymentLinkShare($token){
        $payment_link = PaymentLink::with('user')->where('status', 1)->where('token', $token)->first();

        if(empty($payment_link)){
            return redirect()->route('index')->with(['error' => ['Invalid Payment Link']]);
        }
        ...
        if(empty($credentials)){
            return redirect()->route('index')->with(['error' => ['Can Not Payment Now, Please Contact Support']]);
        }
    ...
    public function gatewayPaymentRequest(Request $request){
        ...
        $payment_link = PaymentLink::with('user')->find($validated['target']);
        if(empty($payment_link)) return back()->with(['error' => ['Invalid Request!']]);
        ...
        if(!$payment_gateway) return back()->with(['error' => ['Gateway Currency Is Not Supported!']]);

        $request->merge(['currency' => $payment_gateway->currency->alias]);

       try {
            $instance = PaymentGatewayHelper::init($request->all())->type(PaymentGatewayConst::TYPEPAYLINK)->gateway()->render();
       } catch (\Exception $e) {
    ...
    public function cardPaymentRequest(Request $request){
        ...
        if(empty($credentials)){
            return back()->with(['error' => ['Transaction Failed, Please Contact Support!']]);
        }
        ...
        $receiver_currency = ExchangeRate::where('name', $payment_link->user->address->country)->first();
        ...
        $sender_currency = ExchangeRate::where('currency_code', $payment_link->currency)->where('currency_name', $payment_link->currency_name)->first();
        ...
        if($payable <= 0 ){
            return back()->with(['error' => ['Transaction Failed, Please Contact With Support!']]);
        }
       ...
       try {
            ...
            return redirect()->route('payment-link.transaction.success', $payment_link->token)->with(['success' => ['Transaction Successful']]);
       } catch (\Exception $e) {
            return back()->with(['error' => ['Something went wrong, Please Try Again!']]);
       }
    }
    ...
    public function stripeSuccess($trx){
        ...
        try{
            PaymentGatewayHelper::init($checkTempData)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('stripe');
        }catch(Exception $e) {
    ...
    public function paypalSuccess(Request $request, $gateway){
        ...
        $checkTempData = $checkTempData->toArray();
        try{
            PaymentGatewayHelper::init($checkTempData)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive();
        }catch(Exception $e) {
            return back()->with(['error' => [$e->getMessage()]]);
        }
    ...
    public function paypalCancel(Request $request, $gateway) {
        ...
        return redirect()->route('index')->with(['error' => ['Transaction Failed. Record didn\'t saved properly. Please try again.']]);;
    }
    ...
    public function flutterwaveCallback($token){
        ...
        if(request()->status == 'successful'){
            ...
            $checkTempData = $checkTempData->toArray();
            try{
                PaymentGatewayHelper::init($checkTempData)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('flutterwave');
            }catch(Exception $e) {
                return back()->with(['error' => [$e->getMessage()]]);
            }
            $payment_link = PaymentLink::find($checkTempData['data']->validated->target);
    ...
    public function sslCommerzSuccess(Request $request){
        ...
        try{
            PaymentGatewayHelper::init($checkTempData)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('sslcommerz');
        }catch(Exception $e) {
            return back()->with(['error' => ["Something Is Wrong..."]]);
        }
    ...
    public function sslCommerzFails(Request $request){
        ...
        if( $data['status'] == "FAILED"){
            ...
            return redirect()->route('index')->with(['error' => ['Transaction Failed Failed']]);
        }
    ...
    public function sslCommerzCancel(Request $request){
        ...
        if( $data['status'] != "VALID"){
            TemporaryData::destroy($checkTempData['id']);
            return redirect()->route('index')->with(['error' => ['Transaction Canceled']]);
        }
    }
    ...
    public function razorPaymentLink($trx_id){
        ...
        return view('frontend.payment.automatic.razor-link', compact('page_title','output','data','orderId'));
    }
    ...
    public function razorCallback()
    {
        ...
        if (isset($request_data['razorpay_order_id'])) {
            ...
            $checkTempData = $checkTempData->toArray();
            try{
                PaymentGatewayHelper::init($checkTempData)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('razorpay');
            }catch(Exception $e) {
    ...
    public function qrpayCallback(Request $request)
    {
        if ($request->type ==  'success') {
            ...
            try {
                PaymentGatewayHelper::init($checkTempData)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('qrpay');
            } catch (Exception $e) {
                return back()->with(['error' => [$e->getMessage()]]);
            }
    ...
    public function qrpayCancel(Request $request, $trx_id)
    {
        ...
        return redirect()->route("index")->with(['error' => ['Payment Canceled']]);
    }
    ...
    public function pagaditoSuccess(){
        ...
        try{
            PaymentGatewayHelper::init($checkTempData)->type($checkTempData['data']->type)->responseReceive('pagadito');

            if($checkTempData['data']->type == PaymentGatewayConst::TYPEPAYLINK){
                $payment_link = PaymentLink::find($checkTempData['data']->validated->target);
                return redirect()->route('payment-link.transaction.success', $payment_link->token)->with(['success' => ['Transaction Successful']]);
            }else{
                $invoice = Invoice::find($checkTempData['data']->validated->target);
                $invoice->update(['status' => 1]);
                return redirect()->route('invoice.transaction.success', $invoice->token)->with(['success' => ['Transaction Successful']]);
            }
        }catch(Exception $e) {
            return back()->with(['error' => [$e->getMessage()]]);
        }
    ...
    public function coinGateSuccess(Request $request, $gateway){
        try{
            ...
            if(Transaction::where('callback_ref', $token)->exists()) {
                if(!$checkTempData) return redirect()->route('payment-link.transaction.success', $payment_link->token)->with(['success' => ['Transaction Successful']]);
            }else {
                if(!$checkTempData) return redirect()->route('index')->with(['error' => [__('Transaction failed. Record didn\'t saved properly. Please try again')]]);
            }
            ...
            PaymentGatewayHelper::init($temp_data)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('coingate');
        }catch(Exception $e) {
    ...
    public function coingateCallback(Request $request,$gateway) {
        ...
        try{
            PaymentGatewayHelper::init([])->type(PaymentGatewayConst::TYPEPAYLINK)->handleCallback($callback_token,$callback_data,$gateway);
        }catch(Exception $e) {
            // handle Error
            logger($e);
        }
    ...
    public function perfectSuccess(Request $request, $gateway){
        try{
            ...
            if(Transaction::where('callback_ref', $token)->exists()) {
                if(!$temp_data) return redirect()->route('payment-link.transaction.success', $payment_link->token)->with(['success' => ['Transaction Successful']]);
            }else {
            ...
            $instance = PaymentGatewayHelper::init($temp_data)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive('perfect-money');
    ...
    public function successGlobal(Request $request, $gateway){
        try{
            ...
            $temp_data = $temp_data->toArray();
            $instance = PaymentGatewayHelper::init($temp_data)->type(PaymentGatewayConst::TYPEPAYLINK)->responseReceive($temp_data['type']);
            if($instance instanceof RedirectResponse) return $instance;
        }catch(Exception $e) {
            return back()->with(['error' => [$e->getMessage()]]);
        }
