...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request){
        ...
        return redirect()->route('user.invoice.preview', $invoice->id)->with(['success' => ['Invoice Created Successful']]);
    }
    ...
    public function update(Request $request){
        ...
        return redirect()->route('user.invoice.preview', $validated['target'])->with(['success' => ['Invoice Updated Successful']]);
    }
    ...
    public function status(Request $request){
        ...
        if($validated['status'] == 3){
            return redirect()->route('user.invoice.index')->with(['success' => ['Invoice Save As Draft']]);
        }else{
            return redirect()->route('user.invoice.share', $invoice->id)->with(['success' => ['Invoice Published Successful']]);
        }
    ...
    public function gatewayPaymentRequest(Request $request){
        ...
        if(empty($invoice)) return back()->with(['error' => ['Invalid Request!']]);
     ...
     */
     ...
     public function cardPaymentRequest(Request $request){
        ...
        if(empty($sender_currency)){
            return back()->with(['error' => ['Sender currency not found!']]);
        }
    ...
    public function delete(Request $request){
        ...
        return back()->with(['success' => ['Invoice Deleted Successful']]);
    }
    ...
    public function coinGateSuccess(Request $request, $gateway){
        try{
            ...
            if(Transaction::where('callback_ref', $token)->exists()) {
                if(!$checkTempData)return redirect()->route('invoice.transaction.success', $invoice->token)->with(['success' => ['Transaction Successful']]);
            }else {
                if(!$checkTempData) return redirect()->route('index')->with(['error' => [__('Transaction failed. Record didn\'t saved properly. Please try again')]]);
            }
    ...
    public function perfectSuccess(Request $request, $gateway){
        try{
            ...
            if(Transaction::where('callback_ref', $token)->exists()) {
                if(!$temp_data) return redirect()->route('invoice.transaction.success', $invoice->token)->with(['success' => ['Transaction Successful']]);
            }else {
