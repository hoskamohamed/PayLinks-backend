...
class AuthorizationController extends Controller
{
    ...
    public function emailResendCode(Request $request)
    {
        ...
        try{
            ...
            DB::commit();
            $message =  ['success'=>['Email varification code resend successfully']];
    ...
    public function verifyEmailCode(Request $request)
    {
     ...
     public function getKycInputFields() {
        ...
        if($user->kyc_verified == GlobalConst::VERIFIED) return ApiResponse::success(['success' => ['You are already KYC Verified User']], $data);
        if($user->kyc_verified == GlobalConst::PENDING) return ApiResponse::success(['success' => ['Your KYC information is submitted. Please wait for admin confirmation']], $data);
    ...
    public function KycSubmit(Request $request) {
        ...
        if($user->kyc_verified == GlobalConst::VERIFIED) return ApiResponse::onlyWarning(['warning' => ['You are already KYC Verified User']]);
        ...
        try{
            ...
            return ApiResponse::onlyError(['success' => ['KYC information successfully submitted']]);
        }

       return ApiResponse::onlySuccess(['success' => ['KYC information successfully submitted']]);
    }
    ...
    public function verify2FACode(Request $request) {
        ...
        if(!$user->two_factor_secret) {
            return ApiResponse::onlyError(['error' => ['Your secret key not stored properly. Please contact with system administrator']]);
        }
        ...
        if(google_2fa_verify($user->two_factor_secret,$code)) {
            ...
            return ApiResponse::onlySuccess(['success' => ['Two factor verified successfully!']]);
        }

        return ApiResponse::onlyError(['error' => ['Failed to login. Please try again']]);
    }
