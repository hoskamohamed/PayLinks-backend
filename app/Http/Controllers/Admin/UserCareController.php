...
class UserCareController extends Controller
{
    ...
    public function userDetails($username)
    {
        ...
        $user = User::where('username', $username)->with('wallet')->first();
        $currency = $user->wallet->currency;
        if(!$user) return back()->with(['error' => ['Oops! User not exists']]);

        $wallet = UserWallet::where('user_id', $user->id)->first();

        $collection_payment = Transaction::where('user_id', $user->id)->payLink()->sum('conversion_payable');
        $collection_invoice = Transaction::where('user_id', $user->id)->payInvoice()->sum('conversion_payable');
        $money_out          = Transaction::where('user_id', $user->id)->moneyOut()->sum('request_amount');
        $total_payment_link = PaymentLink::where('user_id', $user->id)->count();
        $total_invoice      = Invoice::where('user_id', $user->id)->count();

        $data = [
            'wallet'            => $wallet,
            'collection_payment' => $collection_payment,
            'collection_invoice' => $collection_invoice,
            'money_out'          => $money_out,
        ];
