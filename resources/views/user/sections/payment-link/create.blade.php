...
<div class="body-wrapper payment-body-wrapper">
    <div class="custom-card payment-card">
        <div class="payment-type-wrapper">
            ...
            <form action="{{ setRoute('user.payment-link.store') }}" class="payment-form" method="POST" enctype="multipart/form-data">
                @csrf
                                    ...
                                    <select class="select2-auto-tokenize currency_link" name="currency">
                                        <option value="" disabled selected>{{ __('Select One') }}</option>
                                            @foreach ($currency_data as $item)
                                                <option value="{{ $item->code }}" data-country="{{ $item->name }}" data-currency_name="{{ $item->name }}" data-currency_code="{{ $item->code }}" data-currency_symbol="{{ $item->symbol }}">{{ $item->code. ' ('. $item->name.')' }}</option>
                                            @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 form-group">
                                ...
                                <select class="select2-auto-tokenize currency_link_sub" name="sub_currency">
                                    <option value="" disabled selected>{{ __('Select One') }}</option>
                                    @foreach ($currency_data as $item)
                                            <option value="{{ $item->code }}" data-country="{{ $item->name }}" data-currency_name="{{ $item->name }}" data-code="{{ $item->code }}" data-currency_symbol="{{ $item->symbol }}">{{ $item->code. ' ('. $item->name.')' }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <button type="submit" class="btn--base mt-20 active w-100 btn-loading">{{ __('Create New Link') }}</button>
            </form>
        </div>
            ...
            <div class="tab-content" id="nav-tabContent">
                    ...
                    <div class="payment-preview-mockup">
                        ...
                        <div class="payment-preview-box two">
                            <div class="payment-preview-box-left">
                                <h3 class="sub-title "><i class="lab la-windows"></i> <span class="link-sub-title">{{ Auth::user()->address->company_name ?? '' }}</span> <span class="paylink_qty"></span></h3>
                                ...
                                <div class="payment-preview-thumb">
                                    <img src="{{ asset('public/frontend/') }}/images/logo/link_icon.png" alt="logo">
                                </div>
                            </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-xl-12 form-group">
                                            <div class="preview-secure-group">
                                                ...
                                                <p>{{ __('Securely save my inforation for 1-click checkout') }} <span>{{ __('Pay faster on') }} {{ Auth::user()->address->company_name ?? '' }} {{ __('and everywhere Link is accepted') }}</span></p>
                                            </div>
                                        </div>
                ...
                <div class="tab-pane fade show active" id="web" role="tabpanel" aria-labelledby="web-tab">
                    <div class="payment-preview-box">
                        <div class="payment-preview-box-left">
                            <span class="sub-title"><i class="lab la-windows"></i> <span class="link-sub-title">{{ Auth::user()->address->company_name ?? '' }}</span> <span class="paylink_qty"></span></span>
                            <form class="payment-preview-box-left-form">
                                <div class="form-group">
...
<script>
    ...
    $(document).on('keyup change','.sub_price', function(){
        subTotalPaymentCal();
    });
