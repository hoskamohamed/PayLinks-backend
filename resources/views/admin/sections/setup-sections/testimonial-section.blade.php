...
@section('content')
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td>{{ isset($item->language->$system_default_lang->details) ? Str::limit($item->language->$system_default_lang->details, 50, '...') : "" }}</td>
                ...
                <form class="modal-form" method="POST" action="{{ setRoute('admin.setup.sections.section.item.update',$slug) }}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-10-none mt-3">
                        ...
                        <div class="form-group">
                            @include('admin.components.form.input',[
                                'label'     => "Review Rating*",
                                'name'      =>"review_rating_edit",
                                'type'      => 'number',
                                'value'     => old("review_rating_edit",$data->value->review_rating ?? "")
                            ])
                        </div>
                        <div class="col-xl-12 col-lg-12 form-group">
    ...
    <div id="testimonial-view-modal" class="mfp-hide">
        <div class="modal-data">
            <div class="modal-header px-0">
                <h5 class="modal-title">{{ __("View Testimonial") }}</h5>
            </div>
            <div class="modal-form-data">
                <p class="view"></p>
            </div>
...
@push('script')
    ...
    <script>
        ...
        $(".view-modal-button").click(function(){
            var oldData = JSON.parse($(this).parents("tr").attr("data-item"));
            var editModal = $("#testimonial-view-modal");

            editModal.find(".view").text(oldData.language[default_language].details);

            openModalBySelector("#testimonial-view-modal");
        });
