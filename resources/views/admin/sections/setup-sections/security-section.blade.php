...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#security-section-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Item") }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td>
                                    <ul class="user-list">
                                        <li><i class="{{ $item->icon ?? 'fas fa-user' }}"></i></li>
                                    </ul>
                                </td>
                                <td>{{ $item->language->$system_default_lang->name ?? "" }}</td>
                                <td style="width: 50%">{{ $item->language->$system_default_lang->details ? Str::limit($item->language->$system_default_lang->details, 50, '...') : "" }}</td>
                                <td>
    ...
    @include('admin.components.modals.site-section.add-security-section-item')

    {{--  Item Edit Modal --}}
    <div id="security-section-edit" class="mfp-hide large">
        <div class="modal-data">
            <div class="modal-header px-0">
                <h5 class="modal-title">{{ __("Edit Security Section") }}</h5>
            </div>
    ...
    <div id="security-section-view-modal" class="mfp-hide">
        <div class="modal-data">
            <div class="modal-header px-0">
                <h5 class="modal-title">{{ __("View Security") }}</h5>
            </div>
            <div class="modal-form-data">
                <p class="view"></p>
            </div>
...
@push('script')
    ...
    <script>
        openModalWhenError("security-section-add","#security-section-add");
        openModalWhenError("security-section-edit","#security-section-edit");
        ...
        $(".edit-modal-button").click(function(){
            ...
            var editModal = $("#security-section-edit");
            ...
            fileHolderPreviewReInit("#security-section-edit input[name=image]");
            openModalBySelector("#security-section-edit");
        });

        $(".view-modal-button").click(function(){
            var oldData = JSON.parse($(this).parents("tr").attr("data-item"));
            var editModal = $("#security-section-view-modal");

            editModal.find(".view").text(oldData.language[default_language].details);

            openModalBySelector("#security-section-view-modal");
        });
