                ...
                @foreach($recent_journals as $key => $item)
                    <div class="single-popular-item d-flex flex-wrap align-items-center">
                        ...
                        <div class="popular-item-content">
                            ...
                            <h6 class="title"><a href="{{ route('web-journal.details', [$item->id, $item->slug]) }}">{{ $item->title->language->$default_lang->title ? Str::limit($item->title->language->$default_lang->title, 50, '...') : '' }}</a></h6>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="tag-widget-box">
                <ul class="tag-list">
                    @foreach ($journals as $tag_data)
                        @isset($tag_data->tags->language->$default_lang->tags)
                            @foreach ($tag_data->tags->language->$default_lang->tags as $tag)
