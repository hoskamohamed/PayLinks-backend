...
@php
    $default_lang = get_default_language_code() ?? 'en';;
@endphp
@section('content')
                ...
                @foreach($journals as $key => $item)
                        <div class="blog-item mb-30">
                            ...
                            <div class="blog-content">
                                ...
                                <h3 class="title"><a href="{{ setRoute('web-journal.details', [$item->id,$item->slug]) }}">{{ @$item->title->language->$default_lang->title }}</a></h3>
                                @php
                                    $description = strip_tags(@$item->details->language->$default_lang->details)
                                @endphp
